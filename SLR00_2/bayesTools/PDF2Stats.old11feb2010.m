...
if(length(strmatch('bMean', doFields))>0|length(strmatch('bStd', doFields))>0 | strcmp('all',doFields))
    ...
    %stats.bStd = interp1(interpValues,;interpValues,stats.bStd,'nearest').*blank;
    stats.bStd = interp1(interpValues,[interpValues+sqrt(min(binWidthVar))],stats.bStd,'nearest').*blank;
    stats.bStd(end+[1:length(interpValues)])=interpValues;
end
