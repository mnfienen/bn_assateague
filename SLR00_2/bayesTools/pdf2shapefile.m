function pdf2shapefile(Lat,Lon,statstruct, shapename)
...
% get outputs
statnames = fieldnames(statstruct);

% only spit out cases where inputCount>0
if(isfield(statstruct,'inputCount'))
    inputCount = statstruct.inputCount;
...
for k=1:length(statnames)
    ...
    data = getfield(statstruct,statnames{k});
    data = data(doID);
    id = find(isnan(data));
    data(id) = BOGUSVALUE;
    for i=1:Nlatlon
