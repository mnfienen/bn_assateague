...
for mx=1:Mx
    ...
    % speed things up by selecting relevant data now (data can touch the boundary spline and the lagged splines)
    xid = find(abs(x-xm(mx))<5);
    % get x component
    fx = sparse(bspline_basis(x(xid)-xm(mx)));
    if(length(w)==1)
    ...
    if(mx<=2)
        % set it now
        fbx = sparse(bspline_basis(x(xid)-(xm(1)-1)));
    elseif(mx>=(Mx-1))
        % reset it
    ...
    for my=1:My
        % get y-component
        fy = sparse(bspline_basis(y(xid)-ym(my)));
        ...
        % put it together
        f = fx.*fy + bcx(mx)*bcy(my)*fbx.*fby;
        b(id) = f'*(z(xid)); % spline-data covariance
        
        % get covariances
        for ixprime = -3:3
            % the lagged functions first
            if(ixprime==0)
                % fx unchanged
                fxprime = fx;
                fbxprime = fbx;
            else
                % we have moved, get new x-component
                fxprime = sparse(bspline_basis(x(xid)-(xm(mx)-ixprime)));
                % now get new boundary function
                if((mx+ixprime)<=2) % ixprime is 1-sided
                    fbxprime = sparse(bspline_basis(x(xid)-(xm(1)-1)));
                elseif((mx+ixprime)>=(Mx-1))
                    fbxprime = sparse(bspline_basis(x(xid)-(xm(Mx)+1)));
                else
                    fbxprime = 0;
                end
            end
            for iyprime = -3:3
                ...
                if((mx+ixprime)>=1 & (mx+ixprime)<=Mx & (my+iyprime)>=1 & (my+iyprime)<=My & p(id, idprime)==0) 
                    if(iyprime==0)
                        ...
                        % y changed
                        fyprime = sparse(bspline_basis(y(xid)-(ym(my)-iyprime)));
                        % get boundary
                        if((my+iyprime)<=2)
                            fbyprime = sparse(bspline_basis(y(xid)-(ym(1)-1)));
                        elseif((my+iyprime)>=(My-1))
                            fbyprime = sparse(bspline_basis(y(xid)-ym(My)+1));
                        else
                            fbyprime = 0;
                        end
                    end
                    % spline-spline covariance
%                    fprime = fxprime.*fyprime +  bcx(mx+ixprime)*bcy(my+iyprime)*fbxprime.*fbyprime;
