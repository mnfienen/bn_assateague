function [am,aci,J,zm,zs] = bspline_compute(x,y,z,w,xm,ym,lc,bctype);
...
warning('must check that xm includes relevant x')
dxm = diff(xm(1:2));
dxm = [dxm(1);diff(xm(:))];
if(My>1)
dym = diff(ym(1:2));
...
if(1)
    ...
    [junk,gy] = gradient(gy);
    clear junk
    % don't forget to divide by dxtmp^2
    qp = zeros(9,9,9,9);
    
    % now, compute for spline nodes
    warning('q still needs boundary terms')
    %for my1=-4:4
    for my1=5 % just need central
        idy1 = (1+(my1/dxtmp)):(mtmp+1+(my1/dxtmp)); % central id
        for my2=-4:4
            idy2 = (1+(my2/dxtmp)):(mtmp+1+(my2/dxtmp));
            %for mx1=-4:4
            for mx1=5
                idx1 = (1+(mx1/dxtmp)):(mtmp+1+(mx1/dxtmp));
                for mx2=-4:4
                    idx2 = (1+(mx2/dxtmp)):(mtmp+1+(mx2/dxtmp));
                    idx = find(idx1>0 & idx1<=mtmp & idx2>0 & idx2<=mtmp);
                    idy = find(idy1>0 & idy1<=mtmp & idy2>0 & idy2<=mtmp);
                    qp(mx1+5,mx2+5,my1+5,my2+5) = sum(sum(gx(idx1(idx),idy1(idy)).*gx(idx2(idx),idy2(idy)) ...
                        + gy(idx1(idx),idy1(idy)).*gy(idx2(idx),idy2(idy))));
                end
            end
        end
    end
    qp = qp/(mtmp.^2)/(dxtmp.^2);
end
clear gx gy junk
...
for mx=1:Mx
    if(mx==1)
        % set it now
        fbx = sparse(bspline_basis(x-(xm(1)-1)));
    elseif(mx==(Mx-1))
        % reset it
        fbx = sparse(bspline_basis(x-(xm(Mx)+1)));
    else
        %fbx = 0;
    end
    fx = sparse(bspline_basis(x-xm(mx))) + bcx(mx)*fbx;
    ...
    for my=1:My
        ...
        if(my==1)
            % set it
            fby = sparse(bspline_basis(y-(ym(1)-1)));
        elseif(my==(My-1))
            % reset it
            fby = sparse(bspline_basis(y-(ym(My)+1)));
        else
            %fby = 0;
        end
        fy = sparse(bspline_basis(y-ym(my))) + bcy(my)*fby;
        f = fx.*fy;
        b(id) = f'*(z); % spline-data covariance
        ...
        for ixprime = 0:3
            if(ixprime==0)
                % fx unchanged
                fxprime = fx;
            elseif((mx+ixprime)<=Mx)
                %get the correct boundary function at ALL lags
                
                fxprime = sparse(bspline_basis(x-xm(mx)-ixprime)) + bcx(mx+ixprime)*fbx;
            else
                fxprime = 0;
            end
            for iyprime = -3:3
                ...
                if((mx+ixprime)>=1 & (mx+ixprime)<=Mx & (my+iyprime)>=1 & (my+iyprime)<=My & p(id, idprime)==0) 
                    if(iyprime==0)
                        ...
                        fyprime = sparse(bspline_basis(y-ym(my)-iyprime)) + bcy(my+iyprime)*fby;
                    end
                    % spline covariance
                    ...
                    p(id, idprime) = pprime;
                    p(idprime, id) = pprime;
                    % spline-spline covariance, diagonal term
                    qprime = qp(5,5+ixprime,5,5+iyprime);
                    q((mx-1)*My+my, (mx+ixprime-1)*My+(my+iyprime)) = qprime; 
                    q((mx+ixprime-1)*My+(my+iyprime), (mx-1)*My+my) = qprime; 
                end
            end
        end
    end
...
try 
    ...
    am = cgs(r,b); %works, fast too
    aci = cgs(r,ones(Mx*My,1));
end
